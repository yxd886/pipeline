...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
                ...
                same_length=False, clamp_len=-1, untie_r=True,
                use_tpu=False, input_mask=None,
  ...
  if True:
    ...
    for i in range(n_layer):
      # cache new mems
      with tf.variable_scope('transformerxl_layer_{}'.format(i)):
        ...
        scopes.append(tf.get_variable_scope().name)
    with tf.variable_scope("after_attention_layer"):
      if inp_q is not None:
      ...
      scopes.append(tf.get_variable_scope().name)
    return output, new_mems, lookup_table,outputs,scopes
